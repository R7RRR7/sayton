...
#logo
logo = pyfiglet.figlet_format('*      MHMAD      *')
print(X+logo)
print('  ')
print(A+'═'*60)
print('  ')
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('⬩ مسح بيانات البوت ⬩', 'format')],    
         [Button.inline('༺  سورس محمد السيد ༻', 'button0')]
        ]
        await bot.send_message(chat, '''**محمد السيد**''', buttons=buttons)
...
async def back(event):
        ...
        await event.edit("""**محمد السيد**""", buttons=buttons)
...
async def back(event):
        ...
        [Button.inline('نقر زر شفاف ✧', 'ba4utt2on'), Button.inline('عدد الحسابات ꐕ', "bbuttoon08")],
        [Button.inline('⬩ مسح بيانات البوت ⬩', 'format')],    
         [Button.inline('༺ محمد السيد 𖣘 ༻', 'button0')]
        ]
        await event.edit("""**بوت تجميع قناتي @ICTS_930**""", buttons=buttons)
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        await bot.send_message(chat, '''**محمد السيد 𖣘 **''', buttons=buttons)
...
@bot.on(events.NewMessage(pattern='/python', from_users=5825563253))
